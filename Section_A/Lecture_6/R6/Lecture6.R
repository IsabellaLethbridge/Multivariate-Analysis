...
#Deviations between group means and overall mean - for SStr
x_tr <- xl_bar - xbar

#Deviations between observations and group means - for SSres
x1_res <- x1 - xl_bar[1]
x2_res <- x2 - xl_bar[2]
x3_res <- x3 - xl_bar[3]

#Deviations between observations and overall mean - for SStot
x1_tot <- x1 - xbar
x2_tot <- x2 - xbar
x3_tot <- x3 - xbar
...
#Read in data
load('Lecture_6/J_WEx6.11.RData')
...
## Example 6.13
#Read in the data
plastic <- read.table('Lecture_6/T6-4.dat')
head(plastic)
attach(plastic)
X <- cbind(V3, V4, V5)
